...
$utilisateurs = App::getinstance()->getTable('Utilisateur')->allByService($_POST['id']);
...
<form class="form-inline" action="index.php?p=utilisateurs.service" method="POST">
	<select class="form-control" name="id" >
		<?php foreach ($services as $service): ?>
			<option value="<?= $service->id ?>" <?= ($service->id==$_POST['id']) ? 'selected="selected"' : '' ; ?>><?=$service->nom_du_service ?></option>
		<?php endforeach ?>
	</select>
